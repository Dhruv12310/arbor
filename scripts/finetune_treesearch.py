"""
QLoRA fine-tuning for the tree search model.

Usage:
    pip install transformers peft trl datasets accelerate bitsandbytes
    python scripts/finetune_treesearch.py
    python scripts/finetune_treesearch.py --epochs 3 --batch-size 2
"""
...
TRAIN_FILE  = ROOT / "data" / "finetune" / "treesearch_train.jsonl"
EVAL_FILE   = ROOT / "data" / "finetune" / "treesearch_eval.jsonl"
ADAPTER_DIR = ROOT / "models" / "treesearch-adapter"
MERGED_DIR  = ROOT / "models" / "treesearch-merged"
...
def main():
    ...
    if KAGGLE_INPUT.exists():
        TRAIN_FILE  = KAGGLE_INPUT / "treesearch_train.jsonl"
        EVAL_FILE   = KAGGLE_INPUT / "treesearch_eval.jsonl"
        ADAPTER_DIR = Path("/kaggle/working/treesearch-adapter")
        MERGED_DIR  = Path("/kaggle/working/treesearch-merged")

    parser = argparse.ArgumentParser(description="QLoRA fine-tune tree search model")
    parser.add_argument("--epochs",     type=int,   default=3)
    parser.add_argument("--batch-size", type=int,   default=1)
    parser.add_argument("--lr",         type=float, default=2e-4)
    parser.add_argument("--grad-accum", type=int,   default=4)
    parser.add_argument("--hf-repo",    type=str,   default=None,
        help="HuggingFace repo to push merged model e.g. stark12310/arbor-treesearch-3b")
