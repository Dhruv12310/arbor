# Arbor TreeSearch v12 — StructDirect-Trained Navigation Model
# =============================================================
...
#   - StructDirect training data oversampled 1x (was 10x in v11)
#     → 10x oversampling caused memorization; F1 dropped from 0.917 to 0.880
#     → 1x keeps full diversity of the base training set
#   - structure_extractor.py fixed: top-level section page ranges correct
#     → ITEM 8 now spans pages 52-127 instead of 52-52
#   - structure_extractor.py: _fill_parent_gaps() inserted
#     → Unreachable gap pages (e.g. Adobe Item 8 page 58) now navigable
#   - Training data regenerated with fixed extractor (correct physical pages)
#   - OUTPUT_DIR: treesearch-v11 → treesearch-v12
#
# REQUIREMENTS:
#   - Google Colab Pro (A100 40GB recommended)
#   - structdirect_train.jsonl regenerated with updated gen_structdirect_data.py
#   - treesearch_multihop_train.jsonl + treesearch_multihop_eval.jsonl in Drive
#
...
OUTPUT_DIR    = f"{DRIVE_DIR}/models/treesearch-v12"
...
base_train = load_jsonl(TRAIN_FILE)
...
print(f"Combined train (1x SD)     : {len(combined_train)} examples")
...
# model.push_to_hub("TStark12310/arbor-treesearch-v12", private=True)
# tokenizer.push_to_hub("TStark12310/arbor-treesearch-v12", private=True)
"""
...
print(f"\n{'='*45}")
print(f"  TreeSearch v12 — Eval on {n} examples")
print(f"{'='*45}")
...
print(f"{'='*45}")
print(f"  v11         : F1=0.880 | EM=84.5%")
print(f"  v10         : F1=0.917 | EM=87.0%")
print(f"  Target      : F1 ≥ 0.917 | EM ≥ 87%")
print(f"  Status      : {'✓ PASS' if avg_f1 >= 0.917 else '✗ BELOW TARGET'}")
"""
...
from unsloth import FastLanguageModel
MODEL_PATH = "/content/drive/MyDrive/arbor-training-data/models/treesearch-v12"
...
class V12Provider(LLMProvider):
    @property
    def name(self): return "treesearch-v12"
...
async def run_eval():
    ...
    print(f"\n{'='*55}")
    print(f"  StructDirect + TreeSearch v12 — {n} questions")
    print(f"{'='*55}")
    ...
    print(f"{'='*55}")
    print(f"  v11 baseline         : 53.8% (extraction fix, no retrain)")
    print(f"  v10 baseline         : 30.8%")
    print(f"  v9 baseline          : 15.4%")
