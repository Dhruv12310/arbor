# Arbor TreeSearch v14b — StructDirect + DAgger ONLY (no academic data)
# =========================================================================
...
#   - REMOVED academic_train.jsonl (3,829 examples)
#     Root cause of v14 regression: academic = 49.3% of mix, Navigation patterns
#     (Intro/Methods/Results) CONFLICT with financial patterns (ITEM7/ITEM8/FS).
#     Lesson: domain purity. Financial model → financial-only training data.
#   - Training data: StructDirect + DAgger ONLY
#       1. structdirect_train.jsonl  (3,122 examples — 84 FinanceBench docs)
#       2. dagger_corrections.jsonl  (202 examples × 4x oversampled = 808)
#     Total: 3,930 examples
#   - DAgger now = 20.5% of mix (was 10.4% in v14 — stronger recovery signal)
#   - Steps: (3930 × 3) / 16 = 737 steps (shorter run, cleaner signal)
#   - All other hyperparameters identical to v14
#   - load_best_model_at_end=False (already fixed in v14 — keeping)
#   - System prompt normalization (already in v14 — keeping)
#   - Cell 10 PDF-based eval (already fixed in v14 — keeping)
#
# WHY THIS SHOULD BEAT v13 (38%):
#   - DAgger teaches recovery from wrong branch states (teacher forcing fix)
#   - System prompt normalization eliminates train/inference drift
#   - Domain-pure training: model only sees financial navigation patterns
#   - No cross-domain confusion that poisoned v14
#
# DATASETS NEEDED ON DRIVE (MyDrive/arbor-training-data/):
#   - structdirect_train.jsonl  ← already uploaded
#   - dagger_corrections.jsonl  ← already on Drive
#   (academic_train.jsonl is NOT needed — intentionally excluded)
#
# REQUIREMENTS:
#   - Google Colab Pro (A100 40GB recommended)
#   - 2 files in Drive as listed above
#
# EXPECTED RUNTIME on A100: ~10-13 minutes (3,930 examples × 3 epochs)
#   Total steps ≈ (3930 × 3) / 16 = 737 steps
#
# OUTPUT:
#   - Saved to Drive: MyDrive/arbor-training-data/models/treesearch-v14b/
#
...
# Training files — only StructDirect + DAgger (academic intentionally excluded)
STRUCT_FILE  = f"{DRIVE_DIR}/structdirect_train.jsonl"   # 3,122 examples
DAGGER_FILE  = f"{DRIVE_DIR}/dagger_corrections.jsonl"   # 202 examples (4x → 808)

# Eval file — already on Drive (LLM-tree eval, reference only for Cell 7)
EVAL_FILE    = f"{DRIVE_DIR}/treesearch_multihop_eval.jsonl"

OUTPUT_DIR   = f"{DRIVE_DIR}/models/treesearch-v14b"
os.makedirs(OUTPUT_DIR, exist_ok=True)

# Sanity check — verify files exist and count examples
import subprocess
for label, path in [("StructDirect", STRUCT_FILE), ("DAgger", DAGGER_FILE), ("Eval", EVAL_FILE)]:
...
print("NOTE: academic_train.jsonl intentionally NOT loaded (domain mismatch with finance).")
"""
...
# IMPORTANT: Load FRESH base model, NOT treesearch-v13 or v14 adapter
...
# LoRA config — r=16, alpha=16 (same as v14)
...
# ╔══════════════════════════════════════════════════════════╗
# ║  CELL 4 — Load, merge, and format datasets              ║
# ║  KEY CHANGE: StructDirect + DAgger ONLY. No academic.   ║
# ╚══════════════════════════════════════════════════════════╝
"""
import json
import random
...
# 202 × 4 = 808 effective examples = 20.5% of final mix (up from 10.4% in v14)
# More DAgger signal → stronger recovery behavior
random.seed(42)
dagger_oversampled = dagger_raw * 4
random.shuffle(dagger_oversampled)
print(f"  DAgger (4x)  : {len(dagger_oversampled):,} examples")

# ── Merge: StructDirect + DAgger ONLY ─────────────────────────────────────────
# Mix: SD=79.5% (3122), DAgger=20.5% (808) = 3,930 total
# Academic INTENTIONALLY EXCLUDED — 49% academic poisoned v14 (38%→23.2% regression)
all_train = struct_raw + dagger_oversampled  # 3122 + 808 = 3930
print(f"\n  Total before shuffle : {len(all_train):,} examples")
print(f"  Mix: StructDirect={len(struct_raw)/len(all_train)*100:.1f}% | DAgger={len(dagger_oversampled)/len(all_train)*100:.1f}%")
...
# ── Warmup calculation ────────────────────────────────────────────────────────
# Total steps = (3930 examples × 3 epochs) / effective_batch_size(16) = 737 steps
# Warmup = 10% = 74 steps
...
# model.push_to_hub("TStark12310/arbor-treesearch-v14b", private=True)
# tokenizer.push_to_hub("TStark12310/arbor-treesearch-v14b", private=True)
"""
...
DRIVE_DIR  = "/content/drive/MyDrive/arbor-training-data"
MODEL_PATH = f"{DRIVE_DIR}/models/treesearch-v14b"
...
DRIVE_DIR    = "/content/drive/MyDrive/arbor-training-data"
ADAPTER_PATH = f"{DRIVE_DIR}/models/treesearch-v14b"

print("Loading v14b adapter...")
model_test, tokenizer_test = FastLanguageModel.from_pretrained(
...
print(f"  NOTE: v14b dropped academic data; research/multi-domain tests may be slightly")
print(f"        weaker, but finance navigation should be substantially improved.")
print(f"{'='*50}")
"""
...
# ── Step 4: Load v14b adapter ─────────────────────────────────────────────────
from unsloth import FastLanguageModel

DRIVE_DIR    = "/content/drive/MyDrive/arbor-training-data"
ADAPTER_PATH = f"{DRIVE_DIR}/models/treesearch-v14b"

print("Loading v14b model...")
model, tokenizer = FastLanguageModel.from_pretrained(
...
class V14bProvider(LLMProvider):
    @property
    def name(self):
        return "treesearch-v14b"
...
async def run_financebench_v14b():
    results = []
    ...
    print(f"\n{'='*55}")
    print(f"  v14b FinanceBench Retrieval Results")
    ...
    results_path = f"{DRIVE_DIR}/financebench_v14b_results.json"
...
results = await run_financebench_v14b()
"""
