# Arbor TreeSearch v11 — StructDirect-Trained Navigation Model
# =============================================================
...
#   - StructDirect training data oversampled 10x (was 3x)
#     → 136 × 10 = 1360 SD examples = 31% of training (was 4%)
#   - max_seq_length: 2048 → 4096
#     → Fixes Adobe truncation (76-section node lists hit 2212 tokens in v10)
#   - Training data uses fixed gen_structdirect_data.py:
#     → Multi-node navigate_to for evidence spanning 2 sections
#     → Node lists capped at 20 per hop (no truncation during inference either)
#   - warmup_steps recalculated for new dataset size
#   - OUTPUT_DIR: treesearch-v10 → treesearch-v11
#
# REQUIREMENTS:
...
OUTPUT_DIR    = f"{DRIVE_DIR}/models/treesearch-v11"
...
# 10x oversample: SD examples = 31% of training (was 3x = 4% in v10)
combined_train = base_train + sd_train * 10
random.seed(42)
random.shuffle(combined_train)
print(f"Combined train (10x SD)    : {len(combined_train)} examples")
...
# model.push_to_hub("TStark12310/arbor-treesearch-v11", private=True)
# tokenizer.push_to_hub("TStark12310/arbor-treesearch-v11", private=True)
"""
...
print(f"\n{'='*45}")
print(f"  TreeSearch v11 — Eval on {n} examples")
print(f"{'='*45}")
...
print(f"{'='*45}")
print(f"  v10         : F1=0.917 | EM=87.5%")
print(f"  v9 baseline : F1=0.911 | EM=87.0%")
...
from unsloth import FastLanguageModel
MODEL_PATH = "/content/drive/MyDrive/arbor-training-data/models/treesearch-v11"
...
class V11Provider(LLMProvider):
    @property
    def name(self): return "treesearch-v11"
...
async def run_eval():
    ...
    print(f"\n{'='*55}")
    print(f"  StructDirect + TreeSearch v11 — {n} questions")
    print(f"{'='*55}")
